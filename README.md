...
- [linebreak-gate](https://github.com/Baktun-Studio/linebreak-gate) — Fail-closed CI gate for AI-written code: blocks known CVEs (OSV) and serves human-approved specs to the agent over MCP, read-only. Git-only, no account. Install: `uv tool install linebreak-gate && linebreak-gate mcp install --editor claude-code`
