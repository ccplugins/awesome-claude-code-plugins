...
- [MaxAEO AI Visibility](https://github.com/maxaeo/maxaeo-ai-visibility-mcp) — Local-first AI visibility / GEO / AEO audit MCP server. Checks AI crawler access, `robots.txt`, `llms.txt`, sitemap, canonical, metadata, noindex, and JSON-LD, then returns local-only visibility and technical-foundation scores, top issues, and a ranked action plan. No account, API key, domain upload, or telemetry by default. Install: `npx -y maxaeo-ai-visibility-mcp`
