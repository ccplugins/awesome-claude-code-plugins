...
- [metabrain](https://github.com/ariaxhan/metabrain) - Persistent memory for coding agents. Tools: `learn`, `recall`, `verdict`, `hypotheses`, `start_brief`, `stats`, `capture_error`. Stores lessons, failures, and hypotheses in a local SQLite file and retrieves them by keyword before the next task. Install: `uvx --from 'metabrain[mcp]' metabrain-mcp --db ./agent.db`
- [agentmailkit](https://github.com/ariaxhan/agentmailkit) - Email job runner for agents. Tools: `list_jobs`, `run_job` (`dry_run` defaults to true), `preview_job`, `list_plugins`. Defines email sends as named jobs, renders a preview, and only delivers when `dry_run` is set false. Install: `pip install 'agentmailkit[mcp]'` then `agentmailkit mcp`
- [site-spec](https://github.com/ariaxhan/site-spec) - Website audit and repair. Tools: `audit_site`, `fix_issue`, `compile_spec`, `list_checks`. 40 checks across SEO, accessibility, privacy, structured data, and AI searchability; each finding comes back with the file and the fix. Install: `npx -y site-spec-mcp`
